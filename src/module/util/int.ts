import { ErrorGURPS } from "./error.ts"
...
function from(value: number): number {
	return Math.round(value * multiplier()) / multiplier()
}

/* -------------------------------------------- */

function fromString(str: string): [number, Error | null] {
	if (str === "") {
...
function fromStringForced(input: string): number {
...
function extract(input: string): [value: number, remainder: string] {
	let last = 0
