...
export namespace study {
	...
	export namespace Type {
		export function toString(T: Type): string {
			return i18n.localize(`GURPS.Enum.study.Type.${T}.Name`)
		}
		...
		export function limitations(T: Type): string[] {
			switch (T) {
				case Type.Job:
					return [0, 1].map(e => i18n.localize(`GURPS.Enum.study.Type.${T}.Limitations.${e}`))
				default:
					return [0, 1, 2].map(e => i18n.localize(`GURPS.Enum.study.Type.${T}.Limitations.${e}`))
			}
		}

		export function info(T: Type): string {
			const buffer = new StringBuilder()
			const bullet = i18n.localize("GURPS.Tooltip.Prefix")
	...
	export namespace Level {
		export function toString(L: Level): string {
			return i18n.localize(`GURPS.Enum.study.Level.${L}`)
		}
	}
